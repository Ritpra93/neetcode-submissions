class Solution:
    def combinationSum2(self, candidates: List[int], target: int) -> List[List[int]]:
        ...
        #look at optimized solution now since the brute force doesn't pass
        res = []
        candidates.sort()
        #same thing as combinations 1

        def dfs(i, cur, total):
            #dfs like how combinations was
            if total == target:
                res.append(cur.copy())
                return
                #same way combinations 1 was done
            if total > target or i == len(candidates):
                return
